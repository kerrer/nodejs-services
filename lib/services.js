...
var cluster,numCPUs,config,ip,detect;
...
function Services(etcd_host,etcd_port) {
    ...
    services.validate = function(port){
		return  new Promise(function (fullfill, reject){ 
			detect(port, function(error, _port) {
				if (port === _port) {
					fullfill(true);
				}else{
					Log.error('port: %d was occupied, try port: %d', port, _port);
					reject(false);
				}
			});
		}); 
