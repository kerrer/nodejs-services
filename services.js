...
var service = function (server) {
    ...
    this.on('start', function (node) {
        ...
        async.parallel([
                function (callback) {
                    registerServices(server).then(function () {
                        callback(null, 1);
                    }, function (error) {
                        callback(error);
                    });
...
function get(name, param, cb) {
    ...
    async.waterfall([
        function (callback) {
            getNodes(s).then(function (nodes) {
                callback(null, nodes);
            }, function (error) {
                callback(error);
            });
        ...
        function (nodes, callback) {
            getBestNode(nodes).then(function (host) {
                callback(null, host);
            }, function (error) {
                callback(error);
            });
        ...
        function (host, callback) {
            remoteCall(host, param).then(function (result) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        etcd.get(hostUri, function (error, body, header) {
            ...
            async.each(regs, function (key, callback) {
                etcd.del(key, function (error, body, header) {
                    error ? Log.error(error) : null;
                    callback();
                });//etcd.del
            });
